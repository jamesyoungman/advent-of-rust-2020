...
fn get_deck(who: i32, lines: &Vec<String>) -> Result<VecDeque<Card>, String> {
	...
	match PLAYER_RE.captures(line) {
		...
		match cap[1].parse() {
		    ...
		    Err(e) => {
			return Err(format!("cannot parse '{}' as an integer: {}",
					   &cap[1], e));
		    }
		}
	    }
...
impl GameState {
	...
	match who {
	    ...
	    2 => self.p2.score(),
	    _ => {
		panic!(format!("There is no player {}", who));
	    }
	}
    }
	...
	let (c1, c2) = (self.p1.play_next_card(),
			self.p2.play_next_card());
	if c2 > c1 {
	...
	for round in 1.. {
	    ...
	    //assert!(c1 >= 0 && c2 >= 0);
	    let recurse = (c1 as usize) <= self.p1.num_cards() &&
		(c2 as usize) <= self.p2.num_cards();
		...
		*game_counter += 1;
	    ...
	    log::debug!("Player {} wins round {} of game {}!", winner, round, this_game);
	    match winner {
...
fn part1(lines: &Vec<String>) -> Result<(), String> {
    ...
    println!("Part 1: winner is player {} with hand {}; score is {}",
	     winner, hand, score);
    Ok(())
}
...
fn part2(lines: &Vec<String>) -> Result<(), String> {
    ...
    println!("Part 2: winner is player {}; score is {}",
	     winner, score);
    Ok(())
}
