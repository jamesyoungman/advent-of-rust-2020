...
impl BagDefs {
	...
	if !self.definitions.contains_key(colour) {
	    self.definitions.insert(colour.to_string(), Bag::new(colour));
	}
    }
    ...
    fn new() -> BagDefs {
	BagDefs{definitions: HashMap::new()}
    }
	...
	match self.definitions.get(parent_colour) {
	    Some(bag) => bag.required_children.iter()
		.fold(0,
		      |acc, (colour, children)|
		      acc + children * (1 + self.count_children(colour))),
	    None => {
		panic!(format!("we know nothing about what goes into {} bags",
			       parent_colour));
	    }
	}
    }
...
fn parse_line(orig_line: &str) -> (String, HashMap<String, u64>) {
    ...
    if items.is_empty() {
	panic!(format!("every non-empty bag should have contents: '{}' does not",
		       contents_str));
    }
	...
	let cap = match CONTAINS_BAG_RE.captures(item) {
	    None => {
		panic!(format!("content item {} should match CONTAINS_BAG_RE", item));
	    }
	    Some(cap) => cap,
	};
...
fn run() -> Result<(), std::io::Error> {
    ...
    println!("Part 1: a {} bag might be contained in {} bags: {}",
	     sg, parents.len(), parents.join(", "));
    println!("Part 2: a {} bag contains a total of {} other bags",
	     sg, definitions.count_children(sg));
    Ok(())
}
