...
impl GameConsole {
    ...
    fn run_with_change(&mut self, pc: usize, verbose: bool,
		       change: &CodeChange) -> (bool, usize) {
	self.reset(pc);
	loop {
	    if verbose {
		print!("PC={:>4} ACC={:>+4}, executing instruction {}: ",
...
fn part1(console: &mut GameConsole) {
    ...
    println!("Part 1: code {} at PC={} with ACC={:>+4}",
	     (match terminated {
		 true => "terminated normally",
		 false => "entered an infinite loop",
	     }), pc, console.accumulator);
}

fn part2(console: &mut GameConsole) {
    let changes: Vec<CodeChange> = console.code.iter()
	.enumerate()
	.filter(|(_, instr)| match instr.op {
	    Operation::Acc => false,
	    _ => true,
	})
	.map(|(offset, instr)| {
	...
	if terminated {
	    println!("Part 2: console code with flip at {} terminated normally at PC={} with ACC={:>+4}",
		     change.location, pc, console.accumulator);
	    println!("Part 2: code before flip: {}", console.code[change.location]);
