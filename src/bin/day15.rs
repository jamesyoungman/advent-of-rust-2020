...
fn playgame(start_numbers: &[usize], turns: &usize, verbose: bool) -> Option<usize> {
    ...
    for turn in 0..*turns {
        ...
        if turn < start_numbers.len() {
            ...
            turns_spoken.entry(ln).or_insert_with(VecDeque::new);
            let when = turns_spoken.get_mut(&ln).unwrap();
            if when.len() == 1 {
                if verbose {
                    print!(
                        "number {} has never been said before, saying zero; ",
                        last_number.unwrap()
                    );
                }
                this_number = 0;
                ...
                if verbose {
                    print!(
                        "number {} was previously said in turn {}, hence saying {}; ",
                        last_number.unwrap(),
                        earlier,
                        this_number
                    );
...
fn run_one_test(
    ...
    if got != *expected {
        Err(format!(
            "FAIL: '{}': run_one_test: {:?} turn {}: expected {:?}, got {:?}",
...
fn run() -> Result<(), String> {
    ...
    for (part, which_num) in &[(1, 2020), (2, 30000000)] {
        println!(
            "Part {}: number = {:?}",
            part,
            playgame(&start_numbers, which_num, false)
        );
    }
    Ok(())
}
